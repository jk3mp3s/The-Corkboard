...
async function handleDeletePost(postId) {
  if (!isAdminUnlocked) {
    showToast('Unlock admin first', 'error');
    return;
  }

  const target = messages.find(m => m.id === postId);
  if (!target) {
    showToast('Post not found', 'error');
    return;
  }

  const label = target.type === 'photo' ? 'this photo' : 'this note';
  ...
  showToast(`${target.type === 'photo' ? 'Photo' : 'Note'} removed`, 'success');
}

window.deletePostById = async function(postId, event) {
  if (event) {
  ...
  grid.innerHTML = live.map((m, i) => {
    ...
    if (m.type === 'note') return `
      <div class="note ${m.note_class}${adminCls}" style="--rot:${m.rotation}deg;animation-delay:${i * .04}s"
        ...
        ${top}
        ${isAdminUnlocked ? `<button class="item-delete" onclick="deletePostById('${m.id}', event)" title="Delete note">✕</button>` : ''}
...
function renderLightbox() {
  ...
  cardHtml +
  `${isAdminUnlocked ? `<button class="lb-delete show" id="lbDeleteBtn" title="Delete item">🗑</button>` : ''}` +
  `<button class="lb-close" id="lbCloseBtn">✕</button>`;
  const delBtn = document.getElementById('lbDeleteBtn');
  if (delBtn) {
    delBtn.addEventListener('click', async (e) => {
      e.stopPropagation();
      await handleDeletePost(m.id);
    });
  }
