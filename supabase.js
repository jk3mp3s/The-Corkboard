...
/**
 * Delete specific post (used by admin clear board).
 */
export async function dbDeletePost(postId) {
  ...
  if (getErr) {
    console.error('get post for delete:', getErr);
    return false;
  }
  ...
  if (post.type === 'photo') {
    ...
    if (photoErr) {
      console.error('delete photo row:', photoErr);
      return false;
    }
  }
  ...
  if (postErr) {
    console.error('delete post:', postErr);
    return false;
  }
